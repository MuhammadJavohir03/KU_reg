...
// --- 1. O'CHIRISH MANTIQI (Faqat bitta fanni o'chirish) ---
if (isset($_GET['delete_id']) && isset($_GET['column'])) {
    $del_id = (int)$_GET['delete_id'];
    $column = $_GET['column'];

    // Faqat ruxsat berilgan ustunlarni tekshiramiz
    $allowed_columns = ['fan1', 'fan2', 'fan3', 'fan4'];
    if (in_array($column, $allowed_columns)) {
        // Tanlangan fanni NULL qilib yangilaymiz
        $del_stmt = $pdo->prepare("UPDATE pullik SET $column = NULL WHERE id = ?");
        $del_stmt->execute([$del_id]);

        // Agar hamma fanlar bo'sh bo'lib qolsa, qatorni o'chirib yuboramiz
        $check_stmt = $pdo->prepare("SELECT fan1, fan2, fan3, fan4 FROM pullik WHERE id = ?");
        $check_stmt->execute([$del_id]);
        $res = $check_stmt->fetch(PDO::FETCH_ASSOC);

        if (!$res['fan1'] && !$res['fan2'] && !$res['fan3'] && !$res['fan4']) {
            $pdo->prepare("DELETE FROM pullik WHERE id = ?")->execute([$del_id]);
        }

        header("Location: pullik_royhat.php?msg=deleted&q=" . urlencode($_GET['q'] ?? ''));
        exit;
    }
}

// --- 2. SAHIFALASH (PAGINATION) SOZLAMALARI ---
$q = $_GET['q'] ?? '';
...
// --- 3. JAMI QATORLAR SONINI ANIQLASH ---
$countSql = "SELECT COUNT(*) FROM pullik b JOIN users u ON u.id = b.user_id 
...
// --- 4. MA'LUMOTLARNI OLISH (LIMIT BILAN) ---
$sql = "
    SELECT 
        b.*,
        ...
        <div class="dashboard-card">
            ...
            <div class="d-flex flex-column flex-md-row justify-content-between align-items-md-center mb-4 gap-3">
                <div>
                    ...
                    <p class="text-muted small mb-0">Jami topilgan arizalar: <b><?= $total_rows ?></b> ta</p>
                </div>
            </div>
            ...
            <div class="search-section shadow-sm">
                <form method="GET" class="row g-3">
                    ...
                    <div class="col-md-2">
                        <button type="submit" class="btn btn-search btn-custom w-100 shadow-sm"><i class="bi bi-funnel"></i> Qidirish</button>
                    </div>
                    <div class="col-md-3">
                                ...
                                for ($j = 1; $j <= 4; $j++):
                                    ...
                                    if (!empty(trim($row[$fname] ?? ''))): ?>
                                        <tr>
                                            ...
                                            <td class="text-center">
                                                <a href="?delete_id=<?= $row['id'] ?>&column=fan<?= $j ?>&q=<?= urlencode($q) ?>&page=<?= $page ?>"
