...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'add') {
        $stmt = $pdo->prepare("INSERT INTO pullik (talaba_id, familiya, ism, otasi, guruh, yonalish, kurs, parol, fan1, fan2, fan3, created_at)
        ...
        header('Location: pullik_royhat.php?success=add');
        exit;
    }
    ...
    if ($action === 'edit') {
        ...
        if ($id > 0) {
            $stmt = $pdo->prepare("UPDATE pullik SET
                talaba_id = :talaba_id,
                familiya = :familiya,
                ism = :ism,
                otasi = :otasi,
            ...
            header('Location: pullik_royhat.php?success=edit');
            exit;
        }
    }
}
...
if (isset($_GET['delete'])) {
    ...
    if ($id > 0) {
        $stmt = $pdo->prepare("DELETE FROM pullik WHERE id = :id");
        $stmt->execute(['id' => $id]);
    }
    header('Location: pullik_royhat.php?success=delete');
    exit;
}
...
$sql = "SELECT * FROM pullik $filterSql ORDER BY created_at DESC";
...
if (isset($_GET['export']) && $_GET['export'] === 'csv') {
    ...
    header('Content-Disposition: attachment; filename=pullik_royhat_' . date('Ymd_His') . '.csv');
...
<div class="container my-5">
    <div class="d-flex justify-content-between text-white align-items-center mb-4">
        <h2>Pullik ariza ro'yhati</h2>
        <div>
            <a href="pullik_royhat.php?export=csv&search=<?= urlencode($search) ?>" class="btn btn-success">Excelga export (CSV)</a>
                ...
                <?php foreach ($rows as $row): ?>
                    <tr>
                        ...
                        <td>
                            ...
                            <a href="pullik_royhat.php?delete=<?= urlencode($row['id']) ?>" class="btn btn-sm btn-danger" onclick="return confirm('Rostdan ham o`chirmoqchimisiz?');">O'chirish</a>
                        </td>
                    </tr>
                <?php endforeach; ?>
            </tbody>
