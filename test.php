<?php
// ... (Sizning backend kodingiz o'zgarishsiz qoladi)
session_start();
...
<style>
    ...
    .table tbody tr.table-danger-soft {
    ...
    <div class="container-fluid p-4">
        <form method="GET" id="filterForm" class="stat-card shadow-sm p-4 mb-4">
            <div class="row g-3 align-items-end">
                <div class="col-md-2">
                    ...
                    <input list="yonalish_list" name="yonalish" class="form-control select-auto shadow-none" placeholder="Yozing yoki tanlang" value="<?= htmlspecialchars($yonalish) ?>">
                    <datalist id="yonalish_list">
                        <?php foreach ($res_yonalish as $r): ?>
                            <option value="<?= $r['yonalish'] ?>">
                            <?php endforeach; ?>
                    </datalist>
                </div>
                ...
                <div class="col-md-2">
                    ...
                    <input list="guruh_list" name="guruh" class="form-control select-auto shadow-none" placeholder="Yozing yoki tanlang" value="<?= htmlspecialchars($guruh) ?>">
                    <datalist id="guruh_list">
                        <?php foreach ($res_guruh as $r): ?>
                            <option value="<?= $r['guruh'] ?>">
                            <?php endforeach; ?>
                    </datalist>
                </div>
                ...
                <div class="col-md-2">
                    ...
                    <input list="semestr_list" name="semestr" class="form-control select-auto shadow-none" placeholder="Yozing yoki tanlang" value="<?= htmlspecialchars($semestr) ?>">
                    <datalist id="semestr_list">
                        <?php foreach ($res_semestr as $r): ?>
                            <option value="<?= $r['semestr'] ?>">
                            <?php endforeach; ?>
                    </datalist>
                </div>
            ...
            <div class="row g-3 mb-4">
                <div class="col-md-4">
                    <div class="stat-card p-3 d-flex align-items-center justify-content-between shadow-sm">
                        ...
                        <div class="text-end">
                            <div class="circle-stat border-success text-success mb-2"><?= $pass_percent ?>%</div>
                        </div>
                    </div>
                </div>
                <div class="col-md-4">
                    <div class="stat-card p-3 d-flex align-items-center justify-content-between shadow-sm">
                        ...
                        <div class="text-end">
                            <div class="circle-stat border-danger text-danger mb-2"><?= $fail_percent ?>%</div>
                        </div>
                    </div>
                </div>
                <div class="col-md-4">
        ...
        <?php else: ?>
            <div class="alert bg-white border shadow-sm rounded-3 py-4 text-center">
                <h5 class="text-muted mb-0">Ma'lumotlarni ko'rish uchun yo'nalish, guruh va semestrni tanlang.</h5>
            </div>
        <?php endif; ?>
    </div>

    <script>
        // Tanlanganda (yoki yozilgandan keyin ro'yxatdan bosilganda) avtomatik submit qilish
        document.querySelectorAll('.select-auto').forEach(el => {
            ...
            // Status (select) uchun eski holatda qoladi
